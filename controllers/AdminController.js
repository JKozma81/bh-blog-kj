...
class AdminController {
    ...
    return async (req, res) => {
      ...
      if (req.files) {
        await req.files.theme_zip.mv(
          path.join(__dirname, '..', 'themes', req.files.theme_zip.name)
        );
        ...
        if (
          req.files.theme_zip.mimetype !== 'application/x-zip-compressed' ||
  ...
  static addNewAccount(options) {
    ...
    return async (req, res) => {
      try {
        const { user_name, user_password, user_email, roles } = req.body;

    ...
    try {
      ...
      try {
        await this.DBAdapter().runBatchAsync([
          ...
          [`UPDATE slugs SET is_active = 0 WHERE post_id = ?`, postObject.id],
...
/**
 * SELECT posts.id, posts.title, posts.author, posts.content, posts.created_at, posts.draft, posts.published_at, posts.modified_at, slugs.slug_value, GROUP_CONCAT(tags.tag_name, ',') AS tags FROM posts JOIN slugs ON slugs.post_id = posts.id JOIN tags ON posts.id = tags.post_id WHERE posts.id = ? AND slugs.is_active = 1
 */
